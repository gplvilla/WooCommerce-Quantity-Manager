...
class Navigation implements Registerable {
	...
	public function register_navigation_items() {
		...
		Menu::add_plugin_item(
			[
				'id'         => is_null( $this->navigation_id ) ? sanitize_title( $this->plugin->get_name() ) : $this->navigation_id,
				'title'      => is_null( $this->navigation_title ) ? $this->plugin->get_name() : $this->navigation_title,
				'capability' => 'manage_woocommerce',
				'url'        => $this->plugin->get_settings_page_url(),
			]
		);
	}
}
