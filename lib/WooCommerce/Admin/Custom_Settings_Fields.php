...
class Custom_Settings_Fields implements Registerable {
	...
	public function hidden_field( $value ) {
		...
		<input
			...
			<?php echo $custom_attributes; // atts escaped ?> />
		<?php
	...
	public function color_picker_field( $value ) {
		...
		<tr>
			<th scope="row" class="titledesc">
				...
				<?php echo $field_description['tooltip_html']; // escaped ?>
			</th>
			<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?> color-picker-field">
				<input
						type="text"
						name="<?php echo esc_attr( $value['id'] ); ?>"
						id="<?php echo esc_attr( $value['id'] ); ?>"
						...
						class="color-picker <?php echo esc_attr( $value['class'] ); ?>"
					<?php echo $custom_attributes; // escaped ?> />
				<?php echo $field_description['description']; // escaped ?>
			</td>
		</tr>
		<?php
		// phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	public function color_size_field( $value ) {
		...
		$size_placeholder = ! empty( $value['placeholder'] ) ? $value['placeholder'] : __( 'Size', 'woocommerce-quantity-manager' );
		...
		<tr>
			...
			<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?> color-size-field">
				<input
						type="text"
						name="<?php echo esc_attr( $value['id'] . '[color]' ); ?>"
						id="<?php echo esc_attr( $value['id'] . '[color]' ); ?>"
						dir="ltr"
						value="<?php echo esc_attr( $option_value['color'] ); ?>"
						class="color-picker <?php echo esc_attr( $value['class'] ); ?>" />
				<input
						type="number"
	...
	public function image_size_field( $value ) {
		...
		$suffix_html       = ! empty( $value['suffix'] ) ?
			sprintf( '<span class="suffix">%s</span>', esc_html( $value['suffix'] ) ) : '';

		?>
		<tr>
			<th scope="row" class="titledesc">
				<label for="<?php echo esc_attr( $value['id'] ); ?>"><?php echo esc_html( $value['title'] ); ?><?php echo $description['tooltip_html']; ?></label>
			</th>
			<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?> image-size-field">
				<input
						name="<?php echo esc_attr( $value['id'] ); ?>[width]"
						type="number"
						style="<?php echo esc_attr( $value['css'] ); ?>"
						value="<?php echo esc_attr( $current_value['width'] ); ?>"
						class="<?php echo esc_attr( trim( $value['class'] . ' image-width' ) ); ?>"
						placeholder="<?php echo esc_attr( $value['placeholder'] ); ?>"
					<?php echo $custom_attributes; // already escaped ?>
				/>
				...
				<input
						...
						type="number"
						style="<?php echo esc_attr( $value['css'] ); ?>"
						value="<?php echo esc_attr( $current_value['height'] ); ?>"
						class="<?php echo esc_attr( trim( $value['class'] . ' image-height' ) ); ?>"
					<?php echo $custom_attributes; // already escaped ?>
				/><?php echo $suffix_html; ?><?php echo $description['description']; ?>
			</td>
		</tr>
		<?php
	}
	...
	public function register_scripts() {
		wp_register_style( 'barn2-wc-settings', $this->plugin->get_dir_url() . 'lib/assets/css/admin/wc-settings.min.css', [], $this->plugin->get_version() );
		wp_register_script( 'barn2-wc-settings', $this->plugin->get_dir_url() . 'lib/assets/js/admin/wc-settings.min.js', [ 'jquery' ], $this->plugin->get_version() );
	}

	public function load_scripts( $field ) {
		if ( in_array( $field, [ 'image_size', 'color_size', 'color_picker' ], true ) ) {
			wp_enqueue_style( 'barn2-wc-settings' );
		}

		if ( in_array( $field, [ 'color_size', 'color_picker' ], true ) ) {
