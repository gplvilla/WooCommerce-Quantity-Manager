...
namespace Barn2\WQM_Lib\Plugin\License;
...
class Plugin_License implements Registerable, License {
	...
	public function get_status_help_text() {
		...
		switch ( $this->get_status() ) {
			case 'active':
				$message = __( 'Your license key is active.', 'woocommerce-quantity-manager' );
				break;
			case 'inactive':
				if ( ! $message ) {
			...
			default:
				if ( ! $this->get_license_key() ) {
					$message = __( 'Please enter your license key.', 'woocommerce-quantity-manager' );
				} elseif ( ! $message ) {
					$message = __( 'Your license key is invalid.', 'woocommerce-quantity-manager' );
				}
				break;
		}
		...
		switch ( $this->get_error_code() ) {
			case 'missing':
				$message = sprintf(
					...
					__( 'Invalid license key - please check your order confirmation email or %1$sAccount%2$s.', 'woocommerce-quantity-manager' ),
					Util::format_store_link_open( 'account' ),
					'</a>'
				);
				break;
			case 'missing_url':
				$message = __( 'No URL was supplied for activation, please contact support.', 'woocommerce-quantity-manager' );
				break;
			case 'key_mismatch':
				$message = __( 'License key mismatch, please contact support.', 'woocommerce-quantity-manager' );
				break;
			case 'license_not_activable':
				$message = __( 'This license is for a bundled product and cannot be activated.', 'woocommerce-quantity-manager' );
				break;
			case 'item_name_mismatch':
			case 'invalid_item_id':
				$message = __( 'Your license key is not valid for this plugin.', 'woocommerce-quantity-manager' );
				break;
			case 'no_activations_left':
				...
				if ( isset( $license_info['max_sites'] ) ) {
					...
					$limit = sprintf( _n( '%s site active', '%s sites active', absint( $license_info['max_sites'] ), 'woocommerce-quantity-manager' ), $license_info['max_sites'] );
				}

				/* translators: %s: The sites active description, e.g. '2 sites active' */
				$message        = sprintf( __( 'Your license key has reached its activation limit (%s).', 'woocommerce-quantity-manager' ), $limit );
				$read_more_link = Util::format_store_link( 'kb/license-key-problems', __( 'Read more', 'woocommerce-quantity-manager' ) );

				/* translators: support for RTL, 1: the license error, 2: a link */
				$message = sprintf( __( '%1$s %2$s', 'woocommerce-quantity-manager' ), $message, $read_more_link );
				break;
			case 'inactive':
			case 'site_inactive':
				$message = $this->get_license_inactive_message();
			...
			case 'expired':
				...
				if ( ! empty( $license_info['expires'] ) && is_numeric( substr( $license_info['expires'], 0, 4 ) ) ) {
					if ( $expiry_datetime = DateTime::createFromFormat( 'Y-m-d H:i:s', $license_info['expires'] ) ) {
						...
						$message = sprintf( __( 'Your license key expired on %s.', 'woocommerce-quantity-manager' ), $expiry_datetime->format( get_option( 'date_format' ) ) );
					}
				}

				$renewal_link = Util::format_link( $this->get_renewal_url(), __( 'Renew now for 20% discount.', 'woocommerce-quantity-manager' ), true );

				/* translators: support for RTL, 1: the license error, 2: a link */
				$message = sprintf( __( '%1$s %2$s', 'woocommerce-quantity-manager' ), $message, $renewal_link );
				break;
			case 'disabled':
				$message = $this->get_license_disabled_message();
				break;
	...
	private function get_license_inactive_message() {
		return __( 'Your license key is not active. Please reactivate or save the settings.', 'woocommerce-quantity-manager' );
	}

	private function get_license_expired_message() {
		return __( 'Your license key has expired.', 'woocommerce-quantity-manager' );
	}

	private function get_license_disabled_message() {
		return sprintf(
			/* translators: 1: purchase link start, 2: purchase link end. */
			__( 'Your license key has been disabled. Please %1$spurchase a new license key%2$s to continue using the plugin.', 'woocommerce-quantity-manager' ),
