...
namespace Barn2\WQM_Lib\Plugin\License\Admin;

use Barn2\WQM_Lib\Admin\Settings_API_Helper;
use Barn2\WQM_Lib\Plugin\License\License;
...
class License_Key_Setting implements Registerable, License_Setting {
		...
		if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
			...
			if ( isset( $license_setting['license'] ) ) {
				...
				$this->add_settings_message(
					__( 'License key successfully activated.', 'woocommerce-quantity-manager' ),
					__( 'There was an error activating your license key.', 'woocommerce-quantity-manager' ),
					$activated
				);
			}
			...
			$this->add_settings_message(
				__( 'License key successfully deactivated.', 'woocommerce-quantity-manager' ),
				__( 'There was an error deactivating your license key, please try again.', 'woocommerce-quantity-manager' ),
				$deactivated
			);
			...
			$this->add_settings_message(
				__( 'The license key looks good!', 'woocommerce-quantity-manager' ),
				__( 'There\'s a problem with your license key.', 'woocommerce-quantity-manager' ),
	...
	private function is_license_action( $action ) {
		return isset( $_SERVER['REQUEST_METHOD'] ) &&
			...
			( $this->get_license_setting_name() === filter_input( INPUT_POST, $action, FILTER_DEFAULT ) );
	}
	...
	public function get_license_key_setting() {
		$setting = [
			'title' => __( 'License key', 'woocommerce-quantity-manager' ),
			'type'  => 'text',
		...
		if ( $this->is_woocommerce ) {
			$setting['desc_tip'] = __( 'The licence key is contained in your order confirmation email.', 'woocommerce-quantity-manager' );
		} elseif ( $this->is_edd ) {
			// EDD uses title case for setting names, so let's keep things consistent.
			$setting['title'] = __( 'License Key', 'woocommerce-quantity-manager' );
	...
	private function get_license_description() {
		$buttons = [
			'check'      => $this->license_action_button( self::CHECK_KEY, __( 'Check', 'woocommerce-quantity-manager' ) ),
			'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'woocommerce-quantity-manager' ) ),
			'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'woocommerce-quantity-manager' ) )
		];
		...
		if ( $this->license->is_active() ) {
			...
			if ( $this->license->is_inactive() && $this->is_license_action( 'deactivate_key' ) ) {
				...
				$message = __( 'License key deactivated.', 'woocommerce-quantity-manager' );
			} else {
	...
	public function get_license_override_setting() {
		...
		return $override_code ? [
			'type'    => 'hidden',
			'id'      => 'license_override',
			'default' => sanitize_text_field( $override_code )
		] : [];
	}
