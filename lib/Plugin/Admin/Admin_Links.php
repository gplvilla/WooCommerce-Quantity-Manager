...
namespace Barn2\WQM_Lib\Plugin\Admin;

use Barn2\WQM_Lib\Plugin\Plugin;
use Barn2\WQM_Lib\Registerable;
use Barn2\WQM_Lib\Service;
...
class Admin_Links implements Registerable, Service {
	...
	public function add_settings_link( $links ) {
		...
		array_unshift( $links, sprintf( '<a href="%1$s">%2$s</a>', esc_url( $settings_url ), __( 'Settings', 'woocommerce-quantity-manager' ) ) );
	...
	public function add_documentation_link( $links, $file ) {
		...
		$row_meta = [
			'docs' => sprintf(
				...
				esc_attr( sprintf( __( 'View %s documentation', 'woocommerce-quantity-manager' ), $this->plugin->get_name() ) ),
				esc_html__( 'Docs', 'woocommerce-quantity-manager' )
			)
		];
