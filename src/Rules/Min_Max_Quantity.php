 ...
 */
class Min_Max_Quantity extends Abstract_Min_Max_Rule {
	...
	public function __construct( WC_Product $product ) {
		$this->type        = 'quantity_rules';
		$this->data_key    = 'quantity_rules';
		$this->sanitize_cb = [ '\\Barn2\\Plugin\\WC_Quantity_Manager\\Util\\Field', 'sanitize_min_max_quantity' ];
	...
	public function calculate_rules() {
		...
		if ( isset( $this->rule_value['min'] ) ) {
			$this->min = Field_Util::has_meta_value( $this->rule_value['min'] ) ? (int) $this->rule_value['min'] : false;
		}

		if ( isset( $this->rule_value['max'] ) ) {
			$this->max = Field_Util::has_meta_value( $this->rule_value['max'] ) ? (int) $this->rule_value['max'] : false;
		}
	}
